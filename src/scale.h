...
typedef struct {
    ...
    // Refcount: when > 0, active-polling drivers (e.g. G&G) must not
    // send new data requests. Nested pause/resume is supported.
...
// Pause / resume continuous polling (used by G&G and tare paths)
